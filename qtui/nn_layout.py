...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel, QSpinBox, QDoubleSpinBox, QPushButton, QTextEdit, QHBoxLayout, QGroupBox, QApplication
...
class NNLayout:
    ...
    def build(self):
        ...
        controls = {'train_btn': self.train_btn, 'eval_btn': self.eval_btn,
            'lr_input': self.lr_input, 'epochs_input': self.epochs_input,
            'batch_input': self.batch_input, 'threshold_input': self.
            threshold_input, 'results_view': self.results_view}
    ...
    def _on_train(self):
        ...
        self.results_view.append(
            f'Rozpoczynam trening: lr={lr}, epochs={epochs}, batch={batch}')
        threading.Thread(target=self._train_model, args=(lr, epochs, batch),
            daemon=True).start()
    ...
    def _train_model(self, lr, epochs, batch):
        ...
        try:
            ...
            use_tf = False
            self.results_view.append(
                'TensorFlow nie jest zainstalowany, używam sklearn MLPClassifier.'
                )
        ...
        if use_tf:
            model = keras_models.Sequential([keras_layers.Dense(16,
                activation='relu', input_shape=(3,)), keras_layers.Dense(8,
                activation='relu'), keras_layers.Dense(1, activation=
                'sigmoid')])
            model.compile(optimizer=keras_optim.Adam(learning_rate=lr),
                loss='binary_crossentropy', metrics=['accuracy'])
            history = model.fit(X_train, y_train, epochs=epochs, batch_size
                =batch, validation_data=(X_val, y_val), verbose=0,
                callbacks=[tf.keras.callbacks.LambdaCallback(on_epoch_end=
                lambda epoch, logs: self.results_view.append(
                f"Epoka {epoch + 1}/{epochs}: loss={logs['loss']:.4f}, val_loss={logs['val_loss']:.4f}, acc={logs['accuracy']:.4f}, val_acc={logs['val_accuracy']:.4f}"
                ))])
            self.nn_model = model
        else:
            ...
            clf = MLPClassifier(hidden_layer_sizes=(16, 8),
                learning_rate_init=lr, max_iter=epochs, batch_size=batch)
        ...
        self.results_view.append(
            f'Trening zakończony, model zapisany w {model_path}')
    ...
    def _on_evaluate(self):
        ...
        if self.nn_model is None:
            try:
                ...
                self.results_view.append(
                    f'Załadowano model Keras z {model_path}')
            except Exception:
                self.results_view.append(
                    'Brak modelu w pamięci ani pliku. Najpierw uruchom trening.'
                    )
                return
        ...
        html += (
            f'<tr><th>True 0</th><td>{cm[0, 0]}</td><td>{cm[0, 1]}</td></tr>')
        html += (
            f'<tr><th>True 1</th><td>{cm[1, 0]}</td><td>{cm[1, 1]}</td></tr>')
        html += '</table>'
        html += '<h4>Classification Report</h4>'
        html += '<pre>' + report + '</pre>'
        summary = (
            f'Dokładność modelu: {acc:.2%}, AUC: {auc:.2%}. Klasa 0 – precyzja {prec0:.2%}, czułość {rec0:.2%}, F1 {f1_0:.2%}. Klasa 1 – precyzja {prec1:.2%}, czułość {rec1:.2%}, F1 {f1_1:.2%}.'
            )
