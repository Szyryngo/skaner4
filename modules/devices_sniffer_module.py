...
class DevicesSnifferModule(ModuleBase):
    def set_interface(self, iface):
        # Zatrzymaj stary sniffer jeśli istnieje
        if hasattr(self, '_sniffer') and self._sniffer:
            self._sniffer.stop()
        self._iface = iface
        self._sniffer = DevicesSniffer(
            iface=self._iface,
            event_callback=self._on_device_detected
        )
        self._sniffer.start()
    """
    ...
    def _on_device_detected(self, event):
        # Uzupełnij event o last_seen, packets, status
        import datetime
    ...
    def handle_event(self, event):
        # Nie obsługuje eventów z zewnątrz
        pass
