...
class DetectionModule(ModuleBase):
    ...
    def initialize(self, config):
        ...
        self.config = config
        self.if_model_path = os.path.join('data', 'models',
        ...
        try:
            ...
            print(
                f'[DetectionModule] Załadowano model NN z {self.nn_model_path}'
                )
            self.use_nn = True
        except Exception:
            print(
                f'[DetectionModule] Brak modelu NN lub brak TensorFlow, będzie używany IsolationForest'
                )
        ...
        try:
            ...
            print(
                f'[DetectionModule] Załadowano model IF z {self.if_model_path}'
                )
        except Exception:
            ...
            print(
                f'[DetectionModule] Zapisano nowy model IF do {self.if_model_path}'
                )
    ...
    def generate_event(self):
        ...
        if self.use_nn:
            ...
            if prob > 0.5:
                threat = {'ip': features.get('src_ip', 'unknown'),
                    'threat_type': 'anomaly', 'confidence': prob,
                    'ai_weight': prob, 'details': features}
        ...
        if pred == -1:
            threat = {'ip': features.get('src_ip', 'unknown'),
                'threat_type': 'anomaly', 'confidence': float(-score),
                'ai_weight': float(-score), 'details': features}
