...
class UIModule(ModuleBase):
	def _get_real_interfaces(self):
		import psutil
		ignore_keywords = ['virtual', 'vmware', 'loopback', 'bluetooth', 'tunnel', 'pseudo', 'miniport', 'tap', 'vpn', 'docker', 'hyper-v', 'npf']
		interfaces = []
	...
	def _setup_routes(self):
		...
		def config():
			...
			if flask.request.method == "POST":
				...
				if iface:
					# Zapisz wybrany interfejs do config.yaml
					import yaml
		...
		def scanner_scan():
			# Dodaj event SCAN_REQUEST do orchestratora przez specjalny atrybut
			self._scan_request_flag = True
			from flask import redirect, url_for
			return redirect(url_for('scanner'))
	def generate_event(self):
		# Obsługa żądania skanowania z UI
	...
	def _render_nav(self, active, content=None):
		...
		"""
		html = content if content else "<h1>AI Network Packet Analyzer Pro</h1>"
		return nav + html
